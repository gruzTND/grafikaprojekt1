...
def wylicz_wspolrzedne(wymiary, punkt, zoom):
    w, h = wymiary
    x, y = punkt
    zh, zw = zoom
    x *= w/(2*zw); y *= h/(2*zh)
...
def rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom):
    ...
    for o in obiekty:
        punkty = []
        for p in o:
            punkt = wylicz_wspolrzedne(wielkosc_tla, (p[0], p[1]), zoom)
    ...
    while True:
        o = cv2.waitKey(0)
        if o == ord('w'): # w
            obiekty = transformuj(ob, array([-20, 0, 0], float32), array([0, 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('s'): # w
            obiekty = transformuj(ob, array([20, 0, 0], float32), array([0, 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('a'): # w
            obiekty = transformuj(ob, array([0, 20, 0], float32), array([0, 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('d'): # w
            obiekty = transformuj(ob, array([0, -20, 0], float32), array([0, 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('j'): # w
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([0, 0, radians(10)], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('l'): # w
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([0, 0, radians(-10)], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('i'): # w
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([0, radians(10), 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('k'): # w
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([0, radians(-10), 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('q'): # w
            obiekty = transformuj(ob, array([0, 0, -20], float32), array([0, 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('e'): # w
            obiekty = transformuj(ob, array([0, 0, 20], float32), array([0, 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('m'): # w
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([radians(10), 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('n'): # w
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([radians(-10), 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('o'): # w
            zoom[0] += 0.2; zoom[1] += 0.2
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([0, 0, 0], float32), zoom)
            rysuj(obiekty, polaczenia, xyzk, rot, ob, zoom)
        if o == ord('p'): # w
            zoom[0] -= 0.2; zoom[1] -= 0.2
            obiekty = transformuj(ob, array([0, 0, 0], float32), array([0, 0, 0], float32), zoom)
...
def transformuj(ob, xyzk, rot, zoom):
    translacja(ob, xyzk)
    rotacja(ob, rot)
    v = projekcja(ob, zoom)
    return v
...
if __name__ == "__main__":
    ...
    v = transformuj(ob, xyz_kamera, obrot_kamera, [1, 1])
    rysuj(v, polaczenia, xyz_kamera, obrot_kamera, ob, [1,1])